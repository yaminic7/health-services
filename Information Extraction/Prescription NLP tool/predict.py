...
from biobert_ner_ita.utils_ner import (convert_examples_to_features, get_labels, NerTestDataset)
from biobert_ner_ita.utils_ner import InputExample as NerExample
...
BIOBERT_NER_SEQ_LEN = 512
BILSTM_NER_SEQ_LEN = 512
BIOBERT_RE_SEQ_LEN = 512
logging.getLogger('matplotlib.font_manager').disabled = True

LANGUAGE = "fra"

BIOBERT_NER_MODEL_DIR = ""
biobert_ner_labels = ""
biobert_ner_label_map = ""
num_labels_ner = ""

biobert_ner_config = ""
biobert_ner_tokenizer =""
...
def get_biobert_ner_predictions(test_ehr: HealthRecord) -> List[Tuple[str, int, int]]:
    """
    Get predictions for a single EHR record using ita-bert
    ...
    # example : ["B-DRUG","I-DRUG","I-DRUG","I-DRUG","I-DRUG","B-STR","I-STR","B-DOS","B-FOR","B-FRE","I-FRE","O","B-TIME","I-TIME","I-TIME","O"

    predictions = [p for ex in predictions for p in ex]

    #["Lan","##sopra","##zolo","(","lan","##sopra","##zolo","ab","##c",")","30","mg","1","c","##ps","/","die","(","ore","8",")"
    input_tokens = test_ehr.get_tokens()
    prev_pred = ""
    #["B-DRUG","I-DRUG","I-DRUG","I-DRUG","I-DRUG","I-DRUG","I-DRUG","I-DRUG","I-DRUG","I-DRUG","B-STR","I-STR","B-DOS","B-FOR","I-FOR","B-FRE","I-FRE","O","B-TIME","I-TIME","I-TIME","O"
    final_predictions = []
    idx = 0
    ...
    #new predictions for IOB
    IOB = []
    for f, b in zip(input_tokens, final_predictions):
        IOB.append((f ,b))

    #["DRUG",0,9],["STR",10,11],["DOS",12,12],["FOR",13,14],["FRE",15,16],["TIME",18,20]
    ...
    #["DRUG",0,31],["STR",32,37],["DOS",38,39],["FOR",40,43],["FRE",43,47],["TIME",49,55],
...
# noinspection PyTypeChecker
def get_ner_predictions(ehr_record: str, model_name: str, record_id: str = "1") -> HealthRecord:
    """
    Get predictions for NER using either ita-bert or fra-bert or BiLSTM
    ...
    # if LANGUAGE =="ita":
    #     biobert_ner_tokenizer = AutoTokenizer.from_pretrained("dbmdz/bert-base-italian-xxl-cased")
    # else:
    #     biobert_ner_tokenizer = BertTokenizer.from_pretrained("bert-base-multilingual-uncased")
    if model_name.lower() == "ita-bert":
        LANGUAGE = "ita"
        setParameters(LANGUAGE)
    ...
    elif model_name.lower() == "fra-bert":
        #global LANGUAGE
        LANGUAGE = "fra"
        setParameters(LANGUAGE)
        ...
        raise AttributeError("Accepted model names include 'ita-bert' "
                             "'fra-bert'and 'bilstm'.")
    ...
    return test_ehr,predictions

def setParameters(LANGUAGE: str):
    global BIOBERT_NER_MODEL_DIR
    BIOBERT_NER_MODEL_DIR = "biobert_ner_"+LANGUAGE+"/output"

    # =====ita-bert Model for NER======
    global biobert_ner_labels
    biobert_ner_labels = get_labels('biobert_ner_'+LANGUAGE+'/dataset/labels.txt')
    global biobert_ner_label_map
    biobert_ner_label_map = {i: label for i, label in enumerate(biobert_ner_labels)}
    global num_labels_ner
    num_labels_ner = len(biobert_ner_labels)

    global biobert_ner_config
    ...
    global biobert_ner_tokenizer
    if LANGUAGE =="ita":
        #biobert_ner_tokenizer = AutoTokenizer.from_pretrained("dbmdz/bert-base-italian-xxl-cased")
        #biobert_ner_tokenizer.save_pretrained("./models/tokenizer_ita/")
        biobert_ner_tokenizer = AutoTokenizer.from_pretrained("./models/bert-base-italian-xxl-cased/")
    else:
        #biobert_ner_tokenizer = BertTokenizer.from_pretrained("bert-base-multilingual-uncased")
        #biobert_ner_tokenizer.save_pretrained("./models/tokenizer_fra/")
        biobert_ner_tokenizer = AutoTokenizer.from_pretrained("./models/bert-base-multilingual-uncased/")

    global biobert_ner_model
